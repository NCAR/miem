...
namespace miem
{
  ...
  void SpeciesMap::AddMapping(const std::string& inventory_name, const std::string& mechanism_name, Real scaling_factor)
  {
    ...
    for (const auto& [name, total] : scaling_sums)
    {
      if (total > static_cast<Real>(1.0) + static_cast<Real>(1e-6))
      {
        throw MiemException(
            ...
            "Scaling factors for inventory species '" + name + "' sum to " + std::to_string(total) +
                " (>1.0) — mass would be amplified.");
      }
    }
  }

  void SpeciesMap::Apply(
      const std::vector<Real>& inventory_flux,
      ...
      int n_cells) const
  {
    ...
    const std::size_t expected_in = inventory_names.size() * static_cast<std::size_t>(n_cells);
    if (inventory_flux.size() != expected_in)
    {
      throw MiemException(
          MIEM_ERROR_CATEGORY_VALIDATION,
          MIEM_VALIDATION_ERROR_CODE_CELL_COUNT_MISMATCH,
          "SpeciesMap::Apply: inventory_flux size " + std::to_string(inventory_flux.size()) + " does not match " +
              std::to_string(inventory_names.size()) + " species * " + std::to_string(n_cells) + " cells.");
    }
    ...
    mechanism_flux.assign(cached_mechanism_species_.size() * static_cast<std::size_t>(n_cells), static_cast<Real>(0.0));
    ...
    for (const auto& mapping : mappings_)
    {
      ...
      {
        mechanism_flux[dst_row * n_cells + ic] += inventory_flux[src_row * n_cells + ic] * mapping.scaling_factor_;
      }
    }
  }
