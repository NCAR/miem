...
namespace miem
{
  ...
  void Emissions::BuildSources(const std::vector<Source>& sources)
  {
    ...
    for (const auto& src : sources)
    {
      ...
      catch (const MiemException& e)
      {
        ...
        throw MiemException(e.Category(), e.Code(), "source '" + src.name_ + "' failed to construct: " + e.what());
      }
  ...
  void Emissions::ResolveHostIndices(const std::vector<std::string>& host_species, std::vector<int>& indices) const
  {
  ...
  EmissionsState Emissions::Run(double sim_time_sec, double /*dt_sec*/)
  {
      ...
      for (int isp = 0; isp < n_src_sp; ++isp)
      {
        ...
        for (int ic = 0; ic < n_cells_; ++ic)
        {
          mapped[static_cast<std::size_t>(dst_idx) * n_cells_ + ic] =
              src_flux[static_cast<std::size_t>(isp) * n_cells_ + ic] * entry.scaling_factor_;
        }
      }
    ...
    for (int cat : categories)
    {
      ...
      std::sort(
          cat_sources.begin(),
          cat_sources.end(),
          [](const SourceFlux* a, const SourceFlux* b) { return a->hierarchy_ > b->hierarchy_; });

      for (int isp = 0; isp < n_agg; ++isp)
      {
        for (int ic = 0; ic < n_cells_; ++ic)
        {
          const std::size_t idx = static_cast<std::size_t>(isp) * n_cells_ + ic;
