...
void UptempoReader::DetectDimensions()
{
  ...
  if (nc_inq_dimid(ncid_, "nCells", &cell_dim_id_) != NC_NOERR)
  {
    throw MiemException(MIEM_ERROR_CATEGORY_IO,
                        ...
                        "UptempoReader: dimension 'nCells' missing in: " +
                        file_path_);
  }
  std::size_t cell_len;
...
void UptempoReader::DiscoverSpecies()
{
  ...
  for (int varid = 0; varid < n_vars; ++varid)
  {
    ...
    const bool on_mesh_with_time =
        time_dim_id_ >= 0 && ndims == 2 &&
        dimids[0] == time_dim_id_ && dimids[1] == cell_dim_id_;
    const bool on_mesh_snapshot =
        ndims == 1 && dimids[0] == cell_dim_id_;
...
std::vector<double> UptempoReader::GetTimeValues() const
{
  ...
  if (status != NC_NOERR)
  {
    ...
    if (n_time_steps_ > 1)
    {
      throw MiemException(
          ...
          "UptempoReader: file '" + file_path_ + "' has a 'Time' dimension "
          "of length " + std::to_string(n_time_steps_) +
  ...
  const std::string calendar_str =
      ReadTextAttribute(ncid_, xtime_varid, "calendar");
  if (!IsAcceptedCalendar(calendar_str))
  {
    throw MiemException(
        MIEM_ERROR_CATEGORY_IO, MIEM_IO_ERROR_CODE_UNSUPPORTED_CALENDAR,
        "UptempoReader: unsupported time calendar '" + calendar_str +
        "' in " + file_path_ +
  ...
  for (int t = 0; t < n_time_steps_; ++t)
  {
    const std::string stamp =
        TrimFixedWidth(buffer.substr(static_cast<std::size_t>(t) * str_len,
                                     str_len));
    double seconds = 0.0;
    if (!ParseMpasXtime(stamp, seconds))
    {
      throw MiemException(
          ...
          "UptempoReader: could not parse xtime stamp '" + stamp +
          "' in " + file_path_ + " (expected YYYY-MM-DD_HH:MM:SS).");
    }
    times[t] = seconds;
  }
...
void UptempoReader::ReadFlux(int                             time_index,
