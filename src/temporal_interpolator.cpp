...
Result<void> TemporalInterpolator::SetBracket(double                   time_left,
                                              double                   time_right,
                                              ...
                                              const std::vector<Real>& data_right)
{
  if (data_left.size() != data_right.size())
  {
    return Result<void>::Error(
        ...
        "TemporalInterpolator::SetBracket: data_left size " +
        std::to_string(data_left.size()) +
        " != data_right size " + std::to_string(data_right.size()));
  }
                                       ...
                                       std::vector<Real>& output) const
{
  ...
  switch (mode_)
  {
    ...
    case InterpolationMode::kLinear: {
      ...
      {
        output[i] = static_cast<Real>(
            data_left_[i] * (1.0 - alpha) + data_right_[i] * alpha);
      }
      return;
    }
  }
