...
TEST(SpeciesMapApplyTest, A3_UnderUnitySumDropsRemainder)
{
  ...
  std::vector<Real> mech_flux;
  ASSERT_NO_THROW(map.Apply(inv_flux, { "NMVOC" }, mech_flux, n_cells));
...
// ---------------------------------------------------------------------
// A5: Size mismatch -> throws Validation / CELL_COUNT_MISMATCH
// ---------------------------------------------------------------------
TEST(SpeciesMapApplyTest, A5_CellCountMismatchThrows)
{
  SpeciesMap map;
  map.AddMapping("NOx", "NO", 1.0);
  ...
  EXPECT_THROW(
      {
        try
        {
          map.Apply(inv_flux, { "NOx" }, mech_flux, /*n_cells=*/4);
        }
...
TEST(SpeciesMapApplyTest, A6_DistinctInventoryToSameMechanismSums)
{
  ...
  std::vector<Real> mech_flux;
  ASSERT_NO_THROW(map.Apply(inv_flux, { "NOx", "HNO3" }, mech_flux, n_cells));
  ...
  EXPECT_THROW(
      {
        ...
        {
          EXPECT_STREQ(e.category_, MIEM_ERROR_CATEGORY_SPECIES);
          EXPECT_EQ(e.code_, MIEM_SPECIES_ERROR_CODE_SCALING_EXCEEDS_ONE);
          throw;
        }
      },
      miem::MiemException);
