...
//      time steps.
//   2. Assembles an Emissions module (via EmissionsBuilder) with one
//      source: NOx -> NO @ 0.9 + NOx -> NO2 @ 0.1, O3 -> O3 @ 1.0
//      (identity), anthropogenic sector.
...
namespace
{
  ...
  constexpr double kTendTol = std::is_same_v<miem::Real, double> ? 1.0e-25 : 1.0e-18;
...
TEST(NoxPipelineIntegrationTest, NOxSplitAndO3Passthrough)
{
  ...
  {
    EXPECT_NEAR(static_cast<double>(state.surface_flux_(ic, "NO")), 0.9 * kNOxFlux, kFluxTol);   // 9.0e-10
    EXPECT_NEAR(static_cast<double>(state.surface_flux_(ic, "NO2")), 0.1 * kNOxFlux, kFluxTol);  // 1.0e-10
    EXPECT_NEAR(static_cast<double>(state.surface_flux_(ic, "O3")), kO3Flux, kFluxTol);          // 5.0e-10
  }
  ...
  for (int ic = 0; ic < kNCells; ++ic)
  {
    EXPECT_NEAR(static_cast<double>((*sec)(ic, "NO")), 0.9 * kNOxFlux, kFluxTol);
    EXPECT_NEAR(static_cast<double>((*sec)(ic, "NO2")), 0.1 * kNOxFlux, kFluxTol);
    EXPECT_NEAR(static_cast<double>((*sec)(ic, "O3")), kO3Flux, kFluxTol);
  }
}
...
TEST(NoxPipelineIntegrationTest, TendencyMatchesFluxOverColumnIntegrand)
{
  ...
  const auto state = module.Run(/*time_sec=*/1800.0, /*dt=*/60.0, rho.data(), dz.data(), kNVertLevels * kNCells);
  ...
  {
    return state
        .tendency_[static_cast<std::size_t>(isp) * kNVertLevels * kNCells + static_cast<std::size_t>(lv) * kNCells + ic];
  };

  // Layer 0 (surface): non-zero; other layers: zero.
  for (int ic = 0; ic < kNCells; ++ic)
  {
    EXPECT_NEAR(static_cast<double>(tend_at(i_no, 0, ic)), (0.9 * kNOxFlux) / (1.225 * 100.0), kTendTol);
    EXPECT_NEAR(static_cast<double>(tend_at(i_no2, 0, ic)), (0.1 * kNOxFlux) / (1.225 * 100.0), kTendTol);
    EXPECT_NEAR(static_cast<double>(tend_at(i_o3, 0, ic)), kO3Flux / (1.225 * 100.0), kTendTol);
  ...
  for (int isp = 0; isp < static_cast<int>(state.species_names_.size()); ++isp)
  {
    for (int ic = 0; ic < kNCells; ++ic)
    {
      Real sum = Real{ 0 };
      ...
      {
        sum += tend_at(isp, lv, ic) * rho[lv * kNCells + ic] * dz[lv * kNCells + ic];
      }
      const Real flux = state.surface_flux_.At(isp, ic);
      EXPECT_NEAR(static_cast<double>(sum), static_cast<double>(flux), 1.0e-6 * static_cast<double>(flux) + kFluxTol);
    }
  }
}
...
TEST(NoxPipelineIntegrationTest, RepeatedRunsRespectTimeBracket)
{
  ...
  {
    const auto state = module.Run(t, 60.0);
    EXPECT_NEAR(static_cast<double>(state.surface_flux_(0, "NO")), 0.9 * kNOxFlux, kFluxTol);
    EXPECT_NEAR(static_cast<double>(state.surface_flux_(0, "NO2")), 0.1 * kNOxFlux, kFluxTol);
    EXPECT_NEAR(static_cast<double>(state.surface_flux_(0, "O3")), kO3Flux, kFluxTol);
  }
}
